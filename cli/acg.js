#!/usr/bin/env node
/**
 * Copyright 2013-2019 the original author or authors from the acg project.
 *
 * This file is part of the acg project, see https://www.acg.tech/
...
if (process.argv.includes('upgrade') && !process.argv.includes('--prefer-local')) {
    // Prefer global version for `acg upgrade` to get most recent code
    preferLocal = false;
}
...
function requireCLI(preferLocal) {
    ...
    if (preferLocal) {
        try {
            const localCLI = require.resolve(path.join(process.cwd(), 'node_modules', 'generator-acg', 'cli', 'cli.js'));
            if (__dirname !== path.dirname(localCLI)) {
                // load local version
                /* eslint-disable import/no-dynamic-require */
                logger.info("Using acg version installed locally in current project's node_modules");
                require(localCLI);
                return;
            }
        } catch (e) {
    ...
    // load global version
    logger.info('Using acg version installed globally');
    require('./cli');
    /* eslint-enable  */
}
